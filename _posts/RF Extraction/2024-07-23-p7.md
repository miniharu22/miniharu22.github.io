...
- 주파수가 증가함에 따라 모든 Bias Point에서의 \( |h_{21}| \)[dB]이 전부 감소하는 것을 확인 가능  
- \( |h_{21}| \)[dB]이 0dB에 도달하는 지점, 즉 current gain이 '1'이 되는 지점의 주파수가 바로 $$f_t$$  
...
- 이후, 고주파 대역에서는 Oscillation이 일어나지 않고 \( |h_{21}| \)[dB]과 동일하게 Linear하게 감소하는 것이 확인됨  
