...
- **FE 유무에 따른 Variation**   
  - 기본적으로 FE가 있으면 Field가 channel로 전달되기 쉽기 때문에 Variation이 증가   
  - On 특성 : Drift가 주요 메커니즘이므로 $$V_G$$의 영향을 덜 받음 → Variation 小     
  - Off 특성 : Diffusion이 주요 메커니즘이므로 $$V_G$$의 영향을 많이 받음 → Variation 大    

&nbsp;

- **FE 두께에 따른 Variation**   
  - HZO를 포함한 $$T_{ox}$$가 증가할수록 Field가 channel 방향으로 더 확산됨    
  - 강유전층의 두께가 증가하면, 상반된 Dipole의 Misalignment로 인해 field의 세기가 감소    
  - FE가 존재하면, Dipole Alignment가 증가하므로 field의 확산 범위가 감소되는 동시에 field가 채널로 안정적으로 전달됨   
