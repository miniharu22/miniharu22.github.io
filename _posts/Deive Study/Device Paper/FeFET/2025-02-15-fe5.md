...
- **R-FeFET의 MW 측정**   
  ...
  - FE field의 강화와 Charge Trapping 완화로 인해 R-FeFET은 P-FeFET 대비 더 넓은 MW를 보이며, 이때 모든 $$V_{th}$$는 $$I_D$$ = $$\text{10}^{-8}$$ A/um에서 추출됨    
...
- **PGM/ERS Speed 측정**   
  - **$$t_{PGM}$$ & WM**   
    - $$t_{PGM}$$에 따른 Hysteresis transfer curve의 변화를 확인하였고, 이에 따라 R-FeFET과 P-FeFET에 대해 MW를 추출하여 비교를 수행함(Fig.3(c), Fig.3(d))    
    - 그 결과, R-FeFET은 $$t_{PGM}$$ = 50ns일 때도, P-FeFET 대비 약 0.25V 더 넓은 MW를 보임    
  - **PGM/ERS 조건에 따른 $$V_{th}$$ 변화**   
    - R-FeFET과 P-FeFEt의 분극 스위칭 특성을 공정하게 비교하기 위해, 다양한 PGM/ERS 전압과 $$t_{PGM}, t_{ERS}$$ 조건에 대한 $$V_{th}$$ 측정을 수행(Fig.3(e), Fig.3(f))   
    - 결과를 보면, R-FeFET이 모든 조건에 대해 더 빠른 스위칭 속도를 가지고 있음을 확인할 수 있음   

&nbsp;

- **Domain & Domain Wall**   
  - R-FeFET이 P-FeFET보다 더 빠른 Switch speed를 가지는 이유는 **Gate Metal 주변의 FE에 더 강한 field가 인가되어, 극성이 반전된 Domain이 해당 영역에서 더 쉽게 생성(nucleate)되기 때문**    
  - 이때, Domain Wall이 형성되어 Channel 방향으로 확산되는데 따라서 Channel 측의 FE의 field는 P-FeFET과 유사함에도 불구하고 R-FeFET이 더 빠른 PGM Speed를 얻을 수 있는 것    

&nbsp;

### 3-2. Retention & Endurance Analysis   

&nbsp;

<div align="center">
  <img src="/assets/images/Ferro/9.png" width="50%" height="50%" alt=""/>
  <p><em></em></p>
</div>
...
<div align="center">
  <img src="/assets/images/Ferro/10.png" width="60%" height="60%" alt=""/>
  <p><em></em></p>
</div>
...
- **R-FeFET의 우수한 메모리 성능**   
  ...
  - **Metal에서 채널 방향으로 field가 완화되는 구조 덕분에 IL field가 완화되면서** P-FeFET 대비 Retention과 Endurance 특성이 향상됨    
