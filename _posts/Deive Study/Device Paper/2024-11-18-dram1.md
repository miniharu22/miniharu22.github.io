---
layout : single
title: "[BCAT] Trap-Assisted DRAM Row Hammer Effect"
categories: 
  - Device Paper Review
toc: true
toc_sticky: true
...
<div align= 'center'>
    $$N_{RH} = \frac{V_{SN0}-V_{SN-TH}}{\Delta V_{SN}}$$
</div>
...
- **Feauture size Scale down**   
  - 위 Table에서는 Fin width 또는 WL1-WL2 간격이 감소할수록 $$N_{RH}$$ 값이 작아짐을 보여주는데 이는 Feature size의 축소가 Row Hammer effect를 악화시키는 요인임을 시사함   
  - 이는 Trap location matrix에서 $$N_{RH}$$의 비대칭성을 설명하는데, Aggressor WL2와 가까운 지점은 SN1과의 거리가 가까워 carrier migration path가 짧아지므로 $$N_{RH}$$ 값이 상대작으로 작음(135k, 193k)    

&nbsp;

- **Bit Line Junction Depth**   
  - 본 논문에서는 BL Junction Depth를 7nm 증가시킴으로써 Trapped charge의 timing profile에는 영향을 주지 않고 Carrier migration을 조절함    
  - 그 결과, BL Junction Depth가 깊어질수록 Row Hammer effect가 완화되는데, 이는 Bit Line Junction이 깊어질수록 이동 중인 carrier를 더 많이 가로챌 수 있다고 해석됨   
...
<div align="center">
  <img src="/assets/images/AND/38.png" width="50%" height="50%" alt=""/>
  <p><em>Hammering threshold NRH vs temperature from 250 to 350°K for different traps</em></p>
</div>

&nbsp;

- **온도에 따른 $$N_{RH}$$의 변화**  
  - 위 Fig는 온도(250~350k)에 따른 $$N_{RH}$$ 값의 변화를 나타냄    
  - $$N_{RH}$$는 비단조적(non-monotonic) 경향을 보이는데, 상온(300k)에서 최소값에 도달한 후, 온도가 증가/감소할수록 증가하는 경향성을 보임   
  - 또한, $$\Delta N_{RH}$$는 Trap의 위치에 따라 다르게 나타남    

&nbsp;

- **Temperature Effect**   
  - Temperature Effect는 온도에 따른 **Trap Emission rate**와 **Carrier Recombination rate**의 변화로 설명할 수 있음  
  - 높은 온도에서는 Trap emission rate가 증가하여 Trapped charge가 빠르게 손실되며, 이로 인해 이동 가능한 carrier의 수가 감소하여 Row Hammer effect가 감소함($$N_{RH}$$ 증가)   
  - 낮은 온도에서는 Trap emission rate가 감소하여 Trapped charge의 손실이 적고, 이동 가능한 carrier가 많아지지만, 동시에 Recombination rate가 증가하여 Victim SN1으로 이동하는 전자의 수가 줄어듦, 따라서 300k보다 낮은 온도에서도 $$N_{RH}$$가 증가하는 경향을 보임   
