...
- **FeFET-CIM Macro의 동작 과정**   
  ...
  - $$a$$bit Input activation 값들은 IA Driver를 통해 Bit-serial 방식으로 인가되며, 이를 위해서는 $$a$$개의 Clk cycle이 필요함    
  ...
  - 위 과정을 $$a$$개의 cycle동안 반복함으로써 결과적으로 Input Activation 값 IA[$$n_M$$:1]과 weight $$W$$[$$n_M$$:1] 사이의 Dot Product 연산이 마무리됨    
...
<div align="center">
  <img src="/assets/images/Ferro/34.png" width="60%" height="60%" alt=""/>
  <p><em></em></p>
</div>
...
<div align='center'>
    $$P_{SA} = P_{S0}b''n_w + P_{S1}b''(n_w - 1) + P_{S2}b'$$
    $$A_{SA} = (A_{S0}b''n_w)^\epsilon + A_{S1}b''(b_w) + A_{S2}b'$$
</div>
...
- **Shift-and-Add Unit**    
  - Shift-and-Add Unit, 즉 S&A 유닛은 $$n_w$$개의 SAR-ADC 출력값을 입력으로 받아, 각 ADC[$$i$$]의 출력을 $$\frac{w}{n_w}(i - 1)$$bit만큼 shift한 후, 합산함     
    - 해당 결과는 Accumulator에 저장되어 다음 cycle의 결과 누적합산에 사용됨    
  - Fig.4에서 S&A 유닛의 Schematic을 확인 가능하며, Data loss를 방지하기 위해 $$w$$bit weight와 $$a$$bit Input activation 값에 대해 $$b''$$bit shift resister와 $$b'$$bit accumulator를 사용함    
    - Bit $$b'$$와 $$b''$$의 계산에 고려되는 parameter는 다음과 같음    
    - $$n_M$$: 병렬로 access되는 WL의 개수   
    - $$M$$: 전체 FeFET WL의 수    
    - $$w$$: weight bit 수   
    - $$a$$: Input activation bit 수    
  - S&A 유닛의 전력 및 면적 리소스는 위 수식과 같이 계산되며, SAR-ADC 유닛과 마찬가지로 모델링 계수는 Table 1에 정리되어 있음    
  - S&A 유닛의 Latency은 위와 같이 2 clk cycle로 정의됨    

&nbsp;

## 3. Optimal size of the FeFET Sub-array    

&nbsp;

<div align='center'>
    $$P_m = n_Mn_wP_{\text{FeFET}} + MP_{\text{DAC}} + n_WP_{\text{ADC}} + P_{SA} + P_r$$    
    $$A_m = MNA_{\text{FeFET}} + MA_{\text{DAC}} + n_wA_{\text{ADC}} + A_{SA} + A_r$$
</div>

&nbsp;

- **Power & Area Overhead**    
  - FeFET-based CIM Macro의 소비 전력과 면적은 한 cycle에 access되는 $$n_M$$x$$n_w$$ Sub-sarry의 size에 의해 결정됨    
    - 여기서 **$$n_w$$는 하나의 wbit weight를 저장하는 FeFET의 개수**를, **$$n_M$$은 한 cycle에서 동시에 access되는 WL의 수, 즉 연산의 병렬성을 의미함**      
  - 본 논문에서는 CIM Macro의 전력 및 면적에 대한 리소스 $$P_m$$, $$A_m$$을 기준으로 최적의 $$n_M$$, $$n_w$$ 값을 확인하였으며, 각 리소스에 대한 수식은 위와 같음   
    - $$P_r$$과 $$A_r$$은 CIM Macro 내에서 기타 요소들의 리소스로 본 논문의 최적화에서 있어서는 고려 대상이 아니므로 무시됨(negligible)   

&nbsp;

<div align='center'>
    $$\text{PAE} = \frac{2n_M}{P_mA_mT}$$
</div>

&nbsp;

- **Power and Area Efficiency**   
  - 본 논문에서는 CIM Macro의 성능 지표(Figure of Merit, FOM)로써 **전력-면적 효율(Power & Area Efficiency, PAE)**를 위 수식과 같이 정의했으며, 이를 $$n_M$$과 $$n_w$$에 대해 최대화하는 것을 목표로 함   
  - PAE에 대한 평가는 **$$\text{IA}$$[$$n_M$$:1]와 $$W$$[$$n_M$$:1] 간의 Dot product 연산**을 단위 연산으로 고려함   
    - **하나의 MAC 연산은 곱셉과 누적합산으로 구성되며, $$n_M$$개의 MAC는 총 $$2n_M$$개의 연산이 됨**    
  - 따라서 PAE는 위 수식과 같이 **전체 연산 수 대비 전력x면적x시간**으로 정의됨   
    - $$T$$의 경우, 단위 연산의 Latency를 의미   
    - **FeFET Read-out, ADC, SA 동작은 Pipeline 구조로 병렬화가 가능하기 때문에 $$T$$는 결국 $$\text{max}(T_{\text{read}},T_{\text{ADC}},T_{SA})$$로 처리됨**   

&nbsp;

<div align="center">
  <img src="/assets/images/Ferro/36.png" width="60%" height="60%" alt=""/>
  <p><em></em></p>
</div>

&nbsp;

- **PAE를 고려한 최적의 $$n_w$$, $$n_M$$ 분석**    
  - 본 연구에서는 int8과 int4의 weight와 activation, 즉 $$(w,a)$$ = (8,8), (4,4)에 대해 PAE 계산을 수행했으며, 다양한 size의 Sub-array에 대해 실험을 진행함    
    - $$n_w \in {1,2,4,8} for ($$w$$,$$a$$) = (8,8)    
    - $$n_w \in {1,2,4} for ($$w$$,$$a$$) = (4,4)   
    - $$n_M \in {1,2,4,8,16,32,64} for both    
  - PAE 연산에 대한 결과는 Fig.5(a)에 plot되어 있으며 각 경우에 대해서 최적의 $$n_w$$는 다음과 같음   
    - **int8: $$n_w$$ = 4**    
    - **int4: $$n_w$$ = 2**  
    - **즉, 각 FeFET이 2bit의 Partial weight를 저장하는 것이 가장 효율적임**  
  - 최적의 $$n_w$$는 다음의 trade-off를 고려하여 산출됨    
    - **총 ADC의 소비 전력 및 면적 증가 ($$n_w$$ 증가 시, ADC 개수 증가)**    
    - **ADC의 해상도 감소 ($$n_w$$ 증가 시, bit 분할량 감소 → 저해상도 가능)**   
  - 또한 연산의 최적 병렬성, $$n_M$$ 또한 Fig.5에 제시되어 있으며 이 또한 다음의 trade-off를 고려하여 산출됨    
    - **int8: $$n_M$$ = 16**   
    - **int4: $$n_M$$ = 32**   
    - **연산 처리량 증가 ($$n_M$$ 증가 시, 병렬 처리량 증가)**   
    - **ADC의 해상도 증가에 따른 전력, 면적, Latency 증가 (bit 수가 많아질수록 고해상도의 ADC가 필요)**        
