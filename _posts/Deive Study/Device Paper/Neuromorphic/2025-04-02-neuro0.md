...
- **전력 소비량 분석**     
  - $$V_G$$ Ramping 방식과 $$V_D$$ Ramping 방식의 전력 소비를 비교하기 위해, P/D 측정 조건에 대응되는 $$I_G$$-$$V_G$$ 및 $$I_D$$-$$V_D$$ 특성을 측정함          
    - 전력 소비는 $$\Sigma (V_DI_Dt_{\text{width}})$$와 $$\Sigma (V_GI_Gt_{\text{width}})$$로 계산되었으며, $$t_{\text{width}}$$는 100ns의 P/D Pulse width에 해당   
  - $$V_D$$ Ramping 방식은 P/D cycle당 약 $$4.5$$ x $$\text{10}^{-12} J$$의 전력을 소비하지만, $$V_G$$ Ramping 방식은 약 $$2.4$$ x $$\text{10}^{-12} J$$로 거의 절반 수준의 전력을 소비함   
    - 즉, $$V_D$$ Ramping 방식은 더 선형적인 P/D 특성을 얻을 수 있지만, 높은 전력 소비량을 동반함    
    - 물론, Weight update 자체는 자주 발생하는 동작이 아니기 때문에, 전력 소비량이 큰 이슈가 되지 않을 뿐더러, $$V_D$$ Ramping 방식은 동작 속도 변화에 대한 Endurance가 우수하므로 Pulse width($$t_{\text{width}}$$)를 줄임으로써 전력 소비를 절약하는 방법도 있음   

&nbsp;

<div align="center">
  <img src="/assets/images/Ferro/15.png" width="50%" height="50%" alt=""/>
  <p><em></em></p>
</div>

&nbsp;

- **Pattern Recognition 측정**   
  - weight의 선형성과 대칭성이 학습 성능에 미치는 영향을 검증하기 위해 본 논문에서는 Single Layer 및 Double Layer Nerual Network에서 각 훈련 방식에 따른 패턴 인식 Accuracy를 20회 가량 측정함    
    - 그 결과, Linear weight modulation, **즉 $$\Delta w$$ 값이 일정할수록 학습 성능에 직접적인 영향을 준다는 것이 검증됨**   
  - Fig.4(c)와 Fig.4(d)는 $$V_D$$ Ramping 방식이 $$V_G$$ Ramping 방식에 비해 Training Accuracy가 더 높음을 보여주며, 각각의 신경망 구조에 대해 1.44%와 1.48%의 향상을 보임    
    - **특히, 표준편차의 경우 모든 신경망 구조에서 각각 2.49 → 1.74, 0.87 → 0.57로 줄어들었는데, 이는 $$V_D$$ Ramping 방식의 Linear weight update가 weight의 급격한 변화를 억제하기 때문**    
...
- **Incremental Drain Voltage Ramping**   
  - Drain voltage로 인해 생성되는 수평방향의 field로 인해 Channel Region이 감소, 이로 인해 weight의 update가 보다 선형적으로 진행됨    
    - 특히, $$V_G$$ Ramping 방식 대비 높은 Linearity를 보이는데 이는 $$R_{Ch}$$가 국소화됨에 따라 RC Delay가 감소하기 때문    
  - Weight Update의 Linearity가 증가함에 따라 weight 값의 표준편차가 일부 감소   
    - 이로 인해 Backpropagation에서 Vanishing Gradient가 개선됨으로써 Training Accuracy가 개선됨    
