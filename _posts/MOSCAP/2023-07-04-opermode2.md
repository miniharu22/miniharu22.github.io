...
Similarly, the number of holes aggregated on the surface is expressed as $$p_s$$ = $$N_vexp (- \frac{E_f-(E_v-q\pi_s)}{KT}$$ = $$N_Aexp (-\frac{q\pi_s}{KT}$$. In other words, even a slight change in $$\phi_s$$ can be used to infer that the number of holes on the surface changes rapidly.  
